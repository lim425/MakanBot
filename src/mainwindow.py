...
from blynk_thread import BlynkThread
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, app):
        ...
        #  Bylnk Setup
        self.blynk_thread = BlynkThread()
        self.blynk_thread.start()

        # Session Tracking
        self.system_start_time = time.time()
        self.cycle_count = 0
        # Timer to update "Live Time" Blynk
        self.live_time_timer = QTimer()
        self.live_time_timer.setInterval(10000)
        self.live_time_timer.timeout.connect(self.update_blynk_live_time)
        self.live_time_timer.start()
        ...
        self.handle_command("START_UP")
    
    # ========================== Blynk Function ==========================
    
    def update_blynk_live_time(self):
        elapsed_sec = time.time() - self.system_start_time
        elapsed_minutes = elapsed_sec / 60
        time_str = f"{elapsed_minutes:.1f} minutes"
        self.blynk_thread.update_live_time(time_str)

    def trigger_feeding_log(self):        
        elapsed_sec = time.time() - self.system_start_time
        elapsed_minutes = elapsed_sec / 60
        total_time_str = f"{elapsed_minutes:.1f} minutes"
        self.blynk_thread.log_feeding_summary(total_time_str, self.cycle_count)
        self.update_system_status("Blynk Event Logged!")
    ...
    def on_tab_changed(self, index):
        ...
        self.blynk_thread.update_mode(index)
        if index == 1:
            self.start_camera()
        else:
            self.stop_camera()
    ...
    def handle_command(self, command):
        ...
        # --- BLYNK ACTION LOGIC ---
        if command == "HOME":
            self.blynk_thread.update_action(1)
            self.is_at_home = True
            
        elif command == "IDLE":
            self.blynk_thread.update_action(2)           
            
        elif command == "FEED":
            self.blynk_thread.update_action(4)
            self.cycle_count += 1
            self.blynk_thread.update_live_cycle(self.cycle_count)

        elif command == "FINISH":
            self.blynk_thread.update_action(8)
            self.trigger_feeding_log()
            # Stop Everything
            self.live_time_timer.stop()
            self.sequence_timer.stop()
            self.stop_camera()
            self.stop_voice()
            return
        ...
        if command in self.waypoints:
            # Load the data
            target_sequence = self.waypoints[command]

            if self.is_at_home and command != "HOME":
                # Intermediate step to lift arm safely
                transition_step = self.waypoints["START_UP"]
                self.current_waypoints = transition_step + target_sequence
                self.is_at_home = False
            else:
                self.current_waypoints = target_sequence
    ...
    def execute_next_step(self):
        ...
        self.update_system_status(f"Executing...")
    ...
    def closeEvent(self, event):
        ...
        if self.blynk_thread:
            self.blynk_thread.stop()
